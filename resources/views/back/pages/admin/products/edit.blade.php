                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="input-group input-group-outline my-3">
                                    ...
                                    <input type="number" step="0.01" min="0" class="form-control @error('price') is-invalid @enderror" name="price" value="{{ old('price', $product->price) }}">
                                </div>
                                @error('price')
                                    <div class="text-danger text-sm">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-4">
                                <div class="input-group input-group-outline my-3">
                                    ...
                                    <select class="form-control @error('status') is-invalid @enderror" name="status" required>
                                        ...
                                        <option value="sold" {{ old('status', $product->status) == 'sold' ? 'selected' : '' }}>Vendu</option>
                                        <option value="donated" {{ old('status', $product->status) == 'donated' ? 'selected' : '' }}>Donné</option>
                                        <option value="reserved" {{ old('status', $product->status) == 'reserved' ? 'selected' : '' }}>Réservé</option>
                                    </select>
                                </div>
                                @error('status')
                                    <div class="text-danger text-sm">{{ $message }}</div>
                                ...
                                <div class="input-group input-group-outline my-3">
                                    ...
                                    <select class="form-control @error('waste_request_id') is-invalid @enderror" name="waste_request_id">
                                        ...
                                        @foreach($wasteRequests as $wasteRequest)
                                            <option value="{{ $wasteRequest->id }}" {{ old('waste_request_id', $product->waste_request_id) == $wasteRequest->id ? 'selected' : '' }}>{{ $wasteRequest->title }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-12">
                                @if($product->image_path)
                                    <div class="mb-3">
                                        ...
                                        <div>
                                            <img src="{{ asset('storage/' . $product->image_path) }}" alt="Image actuelle" class="img-thumbnail" style="max-width: 200px;">
                                        </div>
                                    </div>
                                @endif
